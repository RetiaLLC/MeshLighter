...
def create_user_pb(node_id_int, long_name, short_name):
    ...
    pb.encode(5, pypb.PB_VARINT, 1) # hw_model = RAK4631
...
def yaoi_fleet_v2(port, count=5, duration=60):
    print(f"Connecting to {port} and launching {count} Yaoi nodes (v2) for {duration} seconds...")
    ...
    while time.time() - start_time < duration:
        ...
        for node in nodes:
            ...
            mesht.send(p_telem)
            time.sleep(1.0) # Longer sleep after full node suite
            
            mesht.update()
        
        time.sleep(2) # Pause between full fleet updates

    print("\nInjection complete.")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python3 yaoi_fleet_v2.py [serial_port]")
        sys.exit(1)
    
    yaoi_fleet_v2(sys.argv[1])
